      ...
      (results, cb) => {
        ...
        if (req.query.dropList) {
          ...
          if (req.query.oname) {
            ...
            whereCondition.oname = { [Op.like]: `%${req.query.oname}%` };
          }
          searchOption = {
            where: whereCondition,
            offset: offset,
        ...
        AuthModel.findAndCountAll(searchOption)
...
function info(req, res) {
  ...
  let tasks = {
    ...
    query: [
      ...
      (results, cb) => {
        // 使用cate的model中的方法查询
        AuthModel.findByPk(req.params.id)
          .then(function (result) {
            // 查询结果处理
            // 如果查询到结果
            if (result) {
...
function add(req, res) {
  ...
  let tasks = {
    ...
    checkParams: (cb) => {
      // 调用公共方法中的校验参数方法，成功继续后面操作，失败则传递错误信息到async最终方法
      Common.checkParams(req.body, ["oname", "pid"], cb);
    },
    // 添加方法，依赖校验参数方法
    add: (cb) => {
...
function update(req, res) {
  ...
  let tasks = {
    ...
    checkParams: (cb) => {
      // 调用公共方法中的校验参数方法，成功继续后面操作，失败则传递错误信息到async最终方法
      Common.checkParams(req.body, ["id", "oname", "pid"], cb);
    },
    // 更新方法，依赖校验参数方法
    update: (cb) => {
      // 使用cate的model中的方法更新
